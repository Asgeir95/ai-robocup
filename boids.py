...
class Bird:
    ...
    def __init__(self):
        self.pos = Vector2D(
            int(random.random() * SCREEN_X), int(random.random() * SCREEN_Y)
        )
...
class Boid(Bird):
    ...
    def draw(self, screen):
        pygame.draw.circle(
            screen, THECOLORS["green"], (int(self.pos.x), int(self.pos.y)), self.radius
        )
        pygame.draw.line(
            screen,
            THECOLORS["blue"],
...
class Hoik(Bird):
    ...
    def draw(self, screen):
        pygame.draw.circle(
            screen, THECOLORS["red"], (int(self.pos.x), int(self.pos.y)), HOIK_RADIUS
        )
        pygame.draw.line(
            screen,
            THECOLORS["white"],
...
class Obstacle:
    ...
    def draw(self, screen):
        pygame.draw.circle(
            screen, THECOLORS["blue"], (int(self.pos.x), int(self.pos.y)), self.radius
        )
...
class Line:
    ...
    def draw(self, screen, direction, length, color=THECOLORS["white"]):
        if direction == HORISONTAL:
            pygame.draw.line(
                screen, color, (self.posx, self.posy), (length, self.posy), 3
            )
        if direction == VERTICAL:
            pygame.draw.line(
                screen, color, (self.posx, self.posy), (self.posx, length), 3
            )
